...
class Monte_Carlo(PricingModel):
    def __init__(self, n_path, t, T):
        self.n_path = n_path
        np.random.seed(42)
        self.Wt = np.random.normal(0, 1 / math.sqrt(360), n_path * (T - t))  # common random numbers, size: self.n_path * _time_step
    ...
    def generate_ST(self, S0, vol, r, T, t, d, q, delta):
        ...
        for i in range(0, T - t):
            dS = ST * (np.ones(self.n_path) * (r - q[i]) + vol*self.Wt[i:i + self.n_path]) - np.ones(self.n_path) * d[i]  # daily increment
