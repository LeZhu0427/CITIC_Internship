...
class Monte_Carlo(PricingModel):
    ...
    def generate_path_dividend_yield(self):
        ...
        for i in range(0, self.T - self.t):
            dS = self.ST * (np.ones(self.n_path) * (self.r - self.dividend[i]) + self.Wt[i:i+self.n_path])      # daily increment
            self.ST = self.ST + dS
            if self.if_barrier:
    ...
    def generate_path_discrete_cash_dividend(self):
        ...
        if_hit = np.ones(self.n_path)     # if barrier is hit
        for i in range(0, self.T - self.t):
            dS = self.ST * (np.ones(self.n_path) * self.r + self.Wt[i:i+self.n_path]) - np.ones(self.n_path) * self.dividend[i]
            self.ST = self.ST+dS
            if self.if_barrier:
                if_hit *= (self.ST < self.B)
            if min(self.ST)<0:
